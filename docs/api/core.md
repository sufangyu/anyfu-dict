...
### `createDictManager<TDict extends RecordBase = DictItemDefault, TResponse = any>(options: DictManagerOptions<TDict, TResponse>)`
...
| 字段                  | 说明                                             | 类型                                                                       | 默认             |
|-----------------------|--------------------------------------------------|----------------------------------------------------------------------------|------------------|
| `baseURL`             | 请求前缀`URL`, 会在`url`不是绝对 URL 时拼接上    | `string`                                                                   | —                |
| `url`                 | 请求 API URL, 绝对 URL 时会忽略`baseURL`         | `string`                                                                   | —                |
| `method`              | HTTP 请求方法                                    | `DictRequestMethod`                                                        | `post`           |
| `headers`             | 自定义请求头                                     | `HeadersInitLike`                                                          | —                |
| `fieldName`           | 提交字典 code 的字段名                           | `string`                                                                   | `codes`          |
| `dictCodeKey`         | 接口返回数据中标识字典 code 的字段名             | `string`                                                                   | `value`          |
| `requestFn`           | 自定义请求函数（可适配 XMLHttpRequest / axios 等） | `(config: RequestConfig) => Promise<TResponse>`                            | 内置 fetch       |
| `requestInterceptor`  | 请求拦截器                                       | `(config: RequestConfig) => RequestConfig \| Promise<RequestConfig>`       | —                |
| `responseInterceptor` | 响应拦截器                                       | `(res: TResponse) => TResponse \| Promise<TResponse>`                      | —                |
| `isSuccess`           | 判断请求是否成功                                 | `(res: TResponse) => boolean`                                              | 内置解析         |
| `parseResponseData`   | 解析实际的响应数据                               | `(res: TResponse) => TDict[]`                                              | 内置解析         |
| `parseDict`           | 解析获取字典数据                                 | `(dictList: TDict[], code: string, codeKey: string) => TDict \| undefined` | 内置解析         |
| `cache`               | 是否启用缓存                                     | `boolean`                                                                  | `true`           |
| `cacheTime`           | 缓存有效时间（毫秒）                               | `number`                                                                   | 1小时            |
| `maxCacheSize`        | 最大缓存条数（LRU 淘汰）                           | `number`                                                                   | `100`            |
| `disabledCacheCodes`  | 永不缓存的字典编码                               | `string[]`                                                                 | `[]`             |
| `mergeDelay`          | 请求合并窗口时间                                 | `number`                                                                   | `25ms`           |
| `retry`               | 失败自动重试次数                                 | `number`                                                                   | `0`              |
| `retryDelay`          | 每次重试间隔                                     | `number`                                                                   | `200ms`          |
| `retryOn`             | 自定义是否触发重试的判断函数                     | `(error: any) => boolean`                                                  | 内置网络错误判断 |
| `fallbackOnError`     | 请求失败时是否返回旧缓存                         | `boolean`                                                                  | `true`           |


### `fetchDict: (code: string | string[], fetchOptions?: FetchDictOptions) => Promise<Record<string, TDict>>`

获取字典数据方法方法, API如下：

| 字段                        | 说明           | 类型                 | 默认 |
|-----------------------------|----------------|----------------------|------|
| `code`                      | 字典编码       | `string \| string[]` | —    |
| `fetchOptions.disableCache` | 强制不使用缓存 | `boolean`            | —    |


### `cancelFetch: (reason?: Error) => void`

取消请求方法, API如下：
| 字段     | 说明           | 类型    | 默认 |
|----------|----------------|---------|------|
| `reason` | 取消请求的原因 | `Error` | —    |


### `deleteCache: (code: string | string[]) => void`
...
### `getDictCache: (code: string) => DictCacheItem | undefined`
